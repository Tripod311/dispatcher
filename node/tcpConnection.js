...
class TCPConnection extends Node {
	...
	onMessage (ev) {
		try {
			if (!rev.data || !rev.data.command) {
				Log.error(`Invalid message format\n${JSON.stringify(rev)}`, 1);
				return;
			}

			switch (rev.data.command) {
				case "ping":
					this.socket.write(JSON.stringify({
						sender: [],
						destination: [],
			...
			let ev = new Event(this.dispatcher, new Address(rev.sender), new Address(rev.destination), rev.data, rev.isResponse, rev.trace);
			ev.dispatch();
		} catch (e) {
